...
### 2026-03-21 delay vs timeout Schema Semantics

- **`delay`** is a pre-execution sleep (applied via `applyStepDelay` before the step body runs). **`timeout`** is an execution deadline (applied via `context.WithTimeout` wrapping the step context). Both are valid, orthogonal Step fields.
- **Critical interaction:** The delay runs *inside* the timeout context, meaning delay eats into the timeout window. With `delay: 5s, timeout: 10s`, the command only gets ~5s. This is non-obvious and undocumented.
- **Unit mismatch:** `delay` allows `ms` unit but `timeout` does not (`^[0-9]+(ms|s|m|h)$` vs `^[0-9]+(s|m|h)$`). Should be unified.
- **Example `argv: ["true"]`** in `edge-case-single-step-timeout` is a Unix-only test placeholder that produces no output, making its `capture: done: stdout` misleading. Not portable to Windows.
- Recommendation: keep both fields, document the interaction, consider applying timeout *after* delay to match Kubernetes probe semantics.
...
### 2026-03-21 UX Feedback Analysis — Single-Step Runbook Review

- **RunbookPanel is a mature three-panel webview** (~4000+ lines) with prose pane (left, tabbed), workflow graph (center, SVG + minimap canvas), and active step detail (right). Panels are resizable via splitters.

- **DisplayConfig is the existing display preference layer.** Defined in `serve/client.ts`, it flows from VS Code settings → `gert serve` start response → UI gating. Already covers: `copySummary`, `saveForReplay`, `debugTrace`, `captures`, `outcomeConditions`, `minimap`, `minimapPosition`, `minimapPanel`, `invokeBoundary`, `hideUnusedSteps`.

- **"Mark Complete" button is hardcoded** at line ~1872: `const btnLabel = isManual ? '✓ Mark Complete' : 'Next Step'`. No customization hook.

- **Minimap glow/pulse** is a `setInterval`-driven alpha modulation in `renderMinimap()` (~line 2234): `alpha = 0.7 + 0.3 * Math.sin(pulsePhase)`. Not gated by any setting.

- **Prose active step indicator** uses a thin 3px `border-left` with `var(--vscode-focusBorder)` and 8% blue background. Very subtle — easy to confuse with content formatting.

- **Summary generation** is `buildSummaryText()` (~230 lines, starts line 2448). Fully hardcoded markdown. Footer: `_Generated by gert runbook engine_` at line 2674. No template or override mechanism.

- **No profile/framework/template concept exists** anywhere in the codebase — not in schemas, not in `gert.yaml`, not in the extension. All configuration is either VS Code settings or hardcoded.

- **Rec/Restart buttons** are always visible; unlike Copy Summary and Save for Replay, they have no toggle setting.

- **Key architectural gap identified:** Need a "profile" layer sitting between project config and individual runbooks. Wrote decision proposal to `.squad/decisions/inbox/gon-ux-profile-framework.md`.

- **Feedback items break into 4 priority tiers:** P0 quick wins (3 items, trivial), P1 UI polish (3 items, medium), P2 layout modes (2 items, hard), P3 architecture (2 items, profile + template systems).

### Deliverables
- `.squad/design/ux-feedback-single-step-2026-03-21.md` — Full 11-item analysis with feasibility, dependencies, priority groupings
- `.squad/decisions/inbox/gon-ux-profile-framework.md` — Profile/framework architectural decision proposal
