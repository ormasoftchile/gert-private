...
\begin{enumerate}[label=\textbf{Q\arabic*.}]
  ...
  \item \textbf{Can the v2 runtime execute v1 runbooks without prior migration?}

    \textit{Why it matters:} Existing gert users have production runbooks in \texttt{runbook/v0} and
    \texttt{runbook/v1} format. If v2 can only execute v2-schema runbooks, adoption requires a
    ...
    \textit{Options:} (a)~Full compatibility mode: v2 runtime transparently executes v1 runbooks by
    internally upgrading them to v2 schema at load time, with no schema file written. (b)~Migration
    gate: v2 refuses to run v1 runbooks and provides a clear error pointing to \texttt{gert migrate}.
    (c)~Opt-in compatibility flag: \texttt{--compat-v1} enables the shim; production is v2-only.
  ...
  \item \textbf{What is the concurrency model for the core runtime?}
    ...
    \textit{Why it matters:} The v1 runtime is single-run-per-process. Whether v2 supports multiple
  ...
  \item \textbf{Is the v2 trace format backward-compatible with the v1 JSONL trace, or is it a new format?}
    ...
    those integrations to be updated before v2 can be rolled out. A backward-compatible superset format
    avoids this but constrains the v2 event schema design.

    \textit{Options:} (a)~New format: v2 defines a clean JSONL schema with a required
    \texttt{schema\_version} field; v1 trace parsers are unsupported. (b)~Backward-compatible superset:
    v2 trace events include all v1 fields plus new fields; existing parsers continue to work on the
    subset they understand. (c)~Parallel trace output: v2 writes both a v2 trace and an optional v1
    ...
    format has fields that would be structurally incompatible with the v2 event model.
  ...
  \item \textbf{Does the \texttt{.provider.yaml} input provider framework survive unchanged in v2?}
    ...
    carried forward as-is, redesigned to fit the v2 component model, or replaced by an extension-based
  ...
  \item \textbf{Is the \texttt{gert serve} JSON-RPC contract a versioned breaking change or an
    ...
    over stdio. If v2 changes the method names, parameter shapes, or notification payloads, the
    ...
    \textit{Options:} (a)~Hard break: v2 defines a new JSON-RPC method set; the VS~Code extension is
    updated as part of the v2 launch. (b)~Versioned negotiation: the \texttt{initialize} handshake
    includes a protocol version field; the host advertises capabilities and the client selects a
    compatible version. (c)~No change: the v1 JSON-RPC surface is preserved unchanged in v2.
    ...
    which methods require changes to expose v2-specific features (e.g., saga events, OTel trace IDs).
  ...
  \item \textbf{Is a gRPC transport for the VS~Code extension viable for v0, or should it be deferred?}
    ...
    \textit{Options:} (a)~Retain JSON-RPC stdio as the sole transport in v2; gRPC deferred to a
  ...
  \item \textbf{What is the parallel execution model for independent steps within a single run?}
    ...
    \textit{Options:} (a)~No parallelism in v2; fan-out deferred to a future version. (b)~Explicit
  ...
  \item \textbf{What is the schema namespace convention for extension-contributed fields, and how are
    ...
    \textit{Why it matters:} The v2 schema design (\S\ref{ch:schema}) states that extension fields
  ...
  \item \textbf{What is the timeout and escalation model for human approval steps?}
    ...
    \textit{Options:} (a)~No timeout in v2; approval escalation deferred. (b)~Optional
    ...
    \textit{Information needed:} Whether the v2 runtime concurrency model (Q2 above) supports
