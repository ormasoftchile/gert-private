...
The gert runtime is deliberately domain-agnostic. It executes steps, enforces
...
\begin{itemize}
  \item The Kit compiler is a \textbf{pure function}: YAML in, YAML out, no
    side effects.
  \item The lowered output is a valid \texttt{runbook/v1} document that uses
...
Gert has three distinct extension mechanisms. Each serves a different purpose
...
You do \emph{not} need to know the gert runtime internals. This guide stands
alone. However, for deeper understanding of the core execution model, consult
the \emph{Gert Design Document} (particularly Chapter~4, Domain Kit Model).
...
For details on the gert runtime, execution model, governance layer, evidence
system, and event model, consult the \emph{Gert Design Document}. In
particular:
