...
\begin{description}
  ...
  \item[\texttt{bin/<compiler-binary>}] --- The Kit compiler. A standalone
    ...
    \texttt{runbook/v1} YAML to stdout. Must be named in the manifest under
...
schema:
  apiVersion-suffix: "compliance/v1"  # Authors use: runbook/v1+compliance/v1
...
\begin{description}
  ...
  \item[\texttt{schema.apiVersion-suffix}] --- The suffix appended to
    \texttt{runbook/v1+} in runbooks authored with this Kit. Example:
    \texttt{compliance/v1} results in \texttt{apiVersion: runbook/v1+compliance/v1}.
...
\begin{itemize}
  ...
  \item Changing the \texttt{apiVersion-suffix} (e.g., \texttt{compliance/v1}
    $\to$ \texttt{compliance/v2 (later major)})
\end{itemize}
...
runbook with \texttt{apiVersion: runbook/v1+compliance/v1}. It reads Kit YAML
on stdin, validates it against the Kit schema, and writes lowered
\texttt{runbook/v1} YAML to stdout.
