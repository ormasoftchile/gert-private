...
\end{minted}
  ...
  \item \textbf{Core planner boundary.}  The gert core planner accepts only
    \texttt{runbook/v2} YAML.  Kit YAML is a pre-processing artefact that
...
\subsubsection{Compiled Output (Core \texttt{runbook/v2} YAML)}
...
// Compile transforms a .home.yaml document into a core runbook/v2 document.
func (c *Compiler) Compile(ctx context.Context, input []byte) ([]byte, error) {
    ...
    // 4. Marshal to YAML and return (valid runbook/v2 YAML).
}
\end{minted}
