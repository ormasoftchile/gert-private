...
\texttt{options\_from} & object & No & Dynamic option list provider for \texttt{select} type \\
\texttt{multiple}   & bool    & No  & Allow multiple selections for \texttt{select} type (default: false) \\
...
\texttt{collector} supports nine field types:
...
\hline
\texttt{text}     & Single-line or multi-line text input & Set \texttt{multiline: true} for textarea \\
...
The optional \texttt{validation} object on a \texttt{text} field supports four
sub-fields:
