...
GXL provides exactly two call forms:
the top-level \texttt{len} builtin and namespace-qualified method calls.
...
All function calls in GXL MUST go through the \texttt{len} builtin or one of
the permitted namespaces (\texttt{str}, \texttt{list}, \texttt{regex}).
...
\texttt{GXL-PARSE-007} & Parser/Lexer & Forbidden syntax: \texttt{\&\&}, \texttt{||}, \texttt{!}, \texttt{|}, ternary, infix \texttt{contains}/\texttt{startsWith}/\texttt{matches}, assignment, string concatenation, array/object construction, negative array index, unary \texttt{+}, or bare function call. \\
\texttt{GXL-PARSE-008} & Parser & Unmatched parenthesis. \\
\texttt{GXL-PARSE-009} & Parser & Chained comparison: e.g., \texttt{a < b < c}. \\
\texttt{GXL-PARSE-010} & Parser & Keyword used as identifier. \\
\hline
