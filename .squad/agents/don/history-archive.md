...
## 2026-08-17 — Runtime Portability Negotiation & Slices 1–2

**Rounds 5–6 Dialogue: Tri-State Defect Discovery**

Verified SQL Live-Site's tri-state defect: `ToolGovernance.RequiresApproval` plain `bool`, no field-presence tracking. Non-nil `*ToolGovernance` proves governance block authored, not that requires-approval was written. Grandfathering rule unimplementable; `*bool` fix necessary and sufficient.

Self-corrected Round 2 claim: approval gate does NOT fire for plain steps from runbook-level governance. `GovernanceEvaluator` never assigned in production wiring (wire.go, run.go). Pre-existing correctness gap; unprompted disclosure to counterparty. Phase 1 constraint: BuildPolicy must compose both runbook and per-tool governance (call-site change, no API change).

**Slice 1: Governance Schema (Commit a2e7db0) — SHIPPED & REVIEWED**

Implemented: `RequiresApproval bool→*bool`, `Classification *string`, `AllowedModes []string` on schema types. Updated read site: `EffectiveGovernanceFromTool()` nil-guard (EffectiveGovernance.RequireApproval stays bool). Added `validateActionClassifications()` validation. Four tri-state unit tests; all pass; zero conformance breaks.

**Learnings:** Schema/effective type split is clean. Pre-existing file state caused git to record file as new even though only 2 lines changed. Commit semantic change is minimal despite large object size.

**Deliverables merged to decisions.md.**

## Learnings

Session: Tri-State RequiresApproval Assessment (2026-08-17T06:37:00-07:00)

- SQL Live-Site defect report was correct. `ToolGovernance.RequiresApproval` is a plain `bool` at `pkg/schema/tool.go:47`. No custom UnmarshalYAML on ToolGovernance (GovernanceConfig has one at runbook.go:107 but only for hyphen/underscore reconciliation). No field-presence mechanism. Our grandfathering rule was unimplementable as published.
- `RequiresApproval *bool` is the correct fix. Schema types (authored) vs. effective/resolved types are separate in the codebase today — `pkg/pkgsubst.EffectiveGovernance` and `pkg/trace.EffectiveGovernancePayload` both have plain `bool RequireApproval`. Pointer only needs to exist on `schema.ToolGovernance`.
- Blast radius is minimal: 1 declaration change + 1 read site (`EffectiveGovernanceFromTool` at pkgsubst.go:359). No Go test literals construct `schema.ToolGovernance{RequiresApproval: ...}` — all test tool defs are YAML-parsed. 0.5 days.
- Runbook-level `GovernanceConfig.RequireApproval` tri-state is over-scoped — pushed back. Classification is per-action; runbook-level require_approval gates the whole run, not individual action safety. Wide blast radius (15+ test sites, 5 read sites in builder/composer/evaluator) for zero benefit.
- No golden file breaks. `tv-enum.yaml`'s 25 `requires-approval: false` entries are input-only YAML strings, not serialized golden output. Nothing in the codebase marshals ToolGovernance to YAML.
- `process.go:StartProcess()` does zero sandboxing: full parent env inherited via `mergeEnv()`, no namespaces, no seccomp, no network isolation, no cwd jail. Subprocess opt-in in test profiles is a trusted assertion, not a technical guarantee. Wording fix needed in profile spec.
