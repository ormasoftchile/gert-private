...
Bundles are distributed via application bundle embedding, over-the-air update channels, or direct download from a registry. The SDK resolves bundle dependencies (platform kits) at load time.
...
The \texttt{manifest.json} file is the only new schema artefact introduced by the mobile execution model. All other files reuse existing gert schemas.
...
\begin{minted}{json}
{
  ...
  "metadata": {
    "description": "Home maintenance kit",
    "author": "Acme Corp",
...
Kits compose linearly: a \textbf{platform kit} provides the device-capability tools (camera, GPS, NFC), and \textbf{application kits} declare runbooks that depend on those tools.
...
The platform kit provides low-level tools that interact with device capabilities (camera, GPS, NFC). Application kits depend on the platform kit and provide runbooks specific to the deploying organisation.
...
Mobile tool definitions extend the \texttt{.tool.yaml} schema (Chapter~\ref{ch:tools}) with an optional \texttt{impl:} block declaring platform-specific handlers.
...
\begin{enumerate}
  ...
  \item One or more \textbf{application kits} providing runbooks (field service, asset inspection, maintenance checklists). These depend on platform kits and are authored by the deploying team.
  \item The mobile application binary, which embeds or downloads these kits at runtime.
\end{enumerate}

The SDK allows multiple application kits to coexist. The application layer presents a unified runbook catalog combining all loaded kits.
...
The mobile execution model is explicitly designed to preserve compatibility with the core gert specification. The following components are \textbf{unchanged}:

\begin{itemize}
  \item \textbf{Runbook YAML format} — \texttt{apiVersion: runbook/v1} documents are identical across CLI, server, and mobile platforms. The same runbook source file compiles to all targets.
...
Kit discovery, dependency resolution, and distribution workflows are covered in Chapter~\ref{sec:kit-catalog}.
