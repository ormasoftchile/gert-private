...
Gert v2 is designed to operate in regulated, high-trust environments where
...
Gert v2 addresses the following threat categories, which are common in
...
\hline
\end{tabular}
\caption{Threat model and mitigations for gert v2.}
...
Gert v2 does not store secrets.  It acts as a pass-through: secrets are
...
Gert v2 supports two primary transport modes for JSON-RPC communication:
...
Gert v2 defines three standard roles:
...
environments.  Gert v2 provides checksum verification and signature validation
...
The \texttt{gert verify} command validates all tools and extensions in the
