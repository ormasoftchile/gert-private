...
Gert v2 is an event-driven system. Every state change in the runtime---run initiation, step advancement, governance checks, tool invocations, human interactions---produces a structured event. Events serve three purposes: (1) adapters (TUI, Web, VS Code) subscribe to events to render UI state without coupling to execution logic, (2) events are appended to the JSONL trace file for audit and replay (§12), and (3) events propagate to observability systems for real-time monitoring and alerting.
...
The following subsections define every normative event kind in v2.0. Each definition includes the payload schema, emission semantics, and whether the event is required or optional.
...
\begin{minted}{json}
{
  ...
  "version":       <string>,  // runbook schema version (e.g., "runbook/v2")
...
\textbf{Emitted:} Before each retry attempt (v2.1 feature; requires retry policy).
...
\textbf{Required:} YES, if retry is attempted. Optional in v2.0 (retry not yet implemented).
...
\subsection{Saga and Compensation Events (v2.1+)}

The following events are reserved for the saga/compensation pattern, which is targeted for v2.1. They are included here for forward compatibility.
...
\textbf{Required:} YES, if compensation is triggered (v2.1+).
...
\paragraph{Implementation.} The HTTP adapter subscribes to \texttt{RunHandle.Events()} and forwards events to all connected WebSocket clients for the given \texttt{run\_id}. The \texttt{/events} endpoint (existing in v1) is upgraded to WebSocket in v2.
...
\texttt{step/retrying} & OPTIONAL & v2.1+ retry feature \\
...
\texttt{saga/compensation\_triggered} & REQUIRED & v2.1+ if saga used \\
\texttt{saga/compensation\_completed} & REQUIRED & v2.1+ if saga used \\
\end{tabular}
\end{center}

\paragraph{Adapter compatibility.} Adapters MUST handle all REQUIRED events. They SHOULD gracefully ignore unknown event kinds to support forward compatibility (e.g., a v2.0 adapter rendering a v2.1 trace with saga events).
