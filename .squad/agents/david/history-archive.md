# david — History Archive

**Archived:** 2026-08-15T16:27:17.3846635-07:00
**Size:** 26620 bytes
...
**2026-08-15 — Dynamic Include Pin Record + Replay + Resume Drift (Stream 4)**

- **Pin record shape:** `LockedDynamicInclude` in `pkg/schema/packagelock.go` carries StepID, RenderedRef, QualifiedID, AbsPath, PackageName, PackageVersion, FileDigest, PackageDigest. Persisted via `PlanMetadata.DynamicIncludes` in `pkg/engine/run.go`.

- **Pinning/drift substrate:** `pkg/pkgdrift/` uses the `DigestPair` / `Evaluate` pattern. For DINC-012 (dynamic include drift), I implemented the check directly rather than reusing `pkgdrift.Evaluate` because Evaluate hard-codes PKG-009. The `checkResumeDynamicIncludeDrift` function in `cmd/gert/resume_drift.go` mirrors `checkResumePackageDrift` exactly — same operator-reporting, same `governance/packageDriftAccepted` audit event, same `--allow-package-drift` flag (B-12: no new flag).

- **Replay re-binding seam:** The `PinBasedIncludeLoader` in `internal/replay/pins.go` implements `internalexecutor.LazyRunbookLoader`. It verifies the on-disk SHA-256 digest of each pinned file and emits `replay/dynamicIncludeDrift` (non-fatal) on mismatch. The loader is ready to be wired by Don (Stream 3) via `RegistryConfig.LazyRunbookLoader` once the DynamicIncludeResolver seam is integrated into the include executor. The `ReplayEngine.WithPins` method accepts pin records from the caller (who loads them from `RunStore.LoadState`).

- **Cross-stream dependency:** `EventKindReplayDynamicIncludeDrift` does not yet exist in `pkg/trace/event.go` (Don's file). A package-local constant `replayDynamicIncludeDriftKind = "replay/dynamicIncludeDrift"` is used as a placeholder. Replace with `tracepkg.EventKindReplayDynamicIncludeDrift` once Don lands Stream 3.

- **B-12 (drift policy):** `--allow-package-drift` covers dynamic include drift — no new flag. Dynamic includes are exports from packages; package drift subsumes them.

- **B-13 (ambiguity state change):** The pin's AbsPath governs replay/resume regardless of current catalog state. `checkResumeDynamicIncludeDrift` only checks `pin.AbsPath` on disk; it does not re-consult the catalog. A test (`TestCheckResumeDynamicIncludeDrift_AmbiguityStateChange_PinAlwaysWins`) covers this case.

- **Drift acceptance is auditable:** Every mismatch that `--allow-package-drift` allows emits `governance/packageDriftAccepted` with `ExpectedCatalogDigest=pin.FileDigest`, `ActualCatalogDigest=current`, `Operator`. Never silent.

**2026-08-15 — Approval conditions B-20/B-21 remediation**

- **Schema description discipline:** When documenting inert or TTY-gated fields, identify every occurrence by surrounding context (sibling field names), not by line number — other agents shift lines during concurrent work. The `when:` case had three inert definitions and one functional one; the functional one (`CollectorField.When`) must be left undescribed as "inert" because it genuinely works. Mistaking line numbers would have misdescribed it.

- **Blast-radius check for schema changes:** Always grep for all occurrences of a key before editing the first one. `require_approval` appeared only once (in `GovernanceConfig`). `when:` appeared four times, only three needing annotation. Never assume a single definition.

- **Verification before application:** Both B-20 conditions were already satisfied when B-21 arrived — Don had reclassified DINC-007 → DINC-W007 repo-wide, and the B-20 schema/example work had landed. Checking before writing saved duplicate work. The discipline is: grep first, apply only what's missing.
