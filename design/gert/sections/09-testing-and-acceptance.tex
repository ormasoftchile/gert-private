...
This section specifies the complete testing strategy for gert: the test framework, unit and contract test patterns, integration test strategy, the \texttt{gert test} feature, regression testing against the v1 runbook corpus, performance acceptance criteria, the extension test harness, and CI/CD gate requirements.
...
v1 scenario files (\texttt{commands}/\texttt{evidence} structure) are forward-compatible with this schema without modification. The parser accepts both the v1 format and the extended extended format (which adds the \texttt{assertions} block). Runbook authors can migrate incrementally by adding \texttt{test.yaml} alongside existing v1 scenarios.
...
The v1 runbook corpus (located in \texttt{examples/} and operator-contributed runbooks) serves as a regression suite for schema compatibility. The regression test runner:

\begin{enumerate}
  \item Reads each v1 runbook from the corpus.
  \item Runs \texttt{gert validate} (schema) and asserts it passes (or passes after migration).
...
Migration compatibility is measured as: the percentage of v1 corpus runbooks that pass \texttt{gert validate} without modification. The schema launch target is $\geq$95\%.
