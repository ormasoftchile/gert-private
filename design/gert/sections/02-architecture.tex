...
This chapter defines the system architecture: the five primary components, their interfaces and invariants,
...
\end{tikzpicture}
\caption{gert architecture --- component dependency overview.}
...
\begin{itemize}
  \item A \texttt{ParsedRunbook} returned without error is structurally valid against the JSON Schema.
...
(parsing and inlining included runbooks whose effective expansion mode is \texttt{eager}; recording
\texttt{lazy} includes as opaque flow nodes for run-time materialization, see
\S\ref{para:planner-expand}), discovers and validates tool definitions, resolves
...
\paragraph{Include expansion modes.}
\label{para:planner-expand}
The Planner consults the active expansion policy (see
\S\ref{para:include-expand}) at every \texttt{include} site. When the
effective mode is \texttt{eager}, the referenced runbook is loaded, planned,
and inlined into the current plan; when it is \texttt{lazy}, the include
appears in the plan as an opaque flow node whose only payload is the absolute
path of the child runbook. Plan-time validation (cycle detection, max-depth,
transitive contracts) walks only the eagerly expanded sub-tree; lazy edges
are validated when they are entered at run time. The Adapter supplies the
Planner with a \emph{lazy loader} that propagates lazy mode recursively into
any sub-include reached through a deferred edge, so a single eager include in
a lazy sub-tree does not silently fall back to eager for the entire chain.
...
embedded JSON Schema, checks structural semantic rules, and returns a \texttt{ParsedRunbook}.
...
\begin{enumerate}
  ...
  \item \textbf{Contract check:} The step's \texttt{Contract.Risk()} is evaluated. Steps
        ...
        ; upgraded to error in strict mode).
...
\begin{itemize}
  ...
  \item \textbf{include:} Resolve the referenced runbook. Validate the include graph for
        ...
        an \texttt{Origin} breadcrumb: \texttt{[included from: <path>]}. Includes may be
        materialized eagerly (loaded and expanded at plan time) or lazily (loaded,
        planned, and inlined when the step is actually entered) according to the
        runbook's \texttt{expand} field, the site's \texttt{include.expand}, or the
        runtime default; see \S\ref{para:include-expand}. Lazy includes carry only
        an absolute path through the plan; at execution time the include executor
        delegates to a sub-engine that reuses the parent run identifier so deferred
        prompts surface on the parent's interaction stream.
...
The \texttt{include} step is the the inline composition primitive. It expands a referenced
...
\texttt{type: include} is the the composition primitive: inline expansion into a shared
...
future; \texttt{type: include} covers the primary composition use case for the 
release.

\subsubsection*{Lexical Tool-Name Scoping}
\label{sec:include-tool-scoping}

Unlike variable scope (which is shared across an include boundary, per the
Key Architectural Properties above), \textbf{tool name binding is lexically
scoped to the runbook file that declares it}
(\texttt{.squad/decisions/archive/barbara-tool-packages-architecture-ruling.md},
AR-TP-8 §8.1, ratified 2026-08-09). An included child runbook does
\textbf{NOT} inherit the parent's \texttt{toolRefs}. If a child runbook's
\texttt{flow} uses \texttt{kubectl}, the child \textbf{MUST} declare
\texttt{kubectl} in its own \texttt{toolRefs}; a \texttt{step.tool.name}
without a same-file \texttt{toolRefs} binding is \texttt{PLAN-010}
(\texttt{design/gert/sections/03d-parse-time-enforcement.tex}
\S Error Code Catalog).

This is a decisive, intentional call, and it is asymmetric with variable
scoping by design: dynamic (inherited) tool-name scoping would make a child
runbook's meaning depend on \emph{who included it}, which destroys both
composability (a child runbook could resolve to a different tool depending
on caller context) and static reachability analysis (the Planner could not
determine, from the child file alone, which tool a step invokes). Variable
scope sharing is safe because captured values are data; tool-name binding
determines \emph{code identity} and must not vary with the call site.

Packages themselves are resolved once, globally, per run
(\texttt{design/gert/sections/06-tool-runtime.tex}
\S Includes, Lexical Scoping, and Global Package Set): the effective package
set is the union of every runbook's \texttt{requires:} in the include
closure, computed at plan time, with intersected version constraints. Only
the \emph{binding} of a bare tool name to a catalog entry
(\texttt{toolRefs:}) is file-local; the underlying catalog is shared.

\subsubsection*{Plan-Time Package Analysis of Lazy Includes}
\label{sec:include-lazy-package-analysis}

The include closure \textbf{MUST} be fully enumerable at plan time even when
individual edges are \texttt{expand: lazy}
(\S\ref{para:planner-expand}). A lazy include is still \emph{resolved and
package-analysed} at plan time: the Planner loads the child runbook far
enough to read its \texttt{requires:} and \texttt{toolRefs:} declarations
and fold them into the global package set and catalog construction
(Phase C, \texttt{design/gert/sections/06-tool-runtime.tex}
\S Tool Discovery and Catalog Construction), even though the child's
\emph{steps} are only expanded into the execution queue at run time.

A runtime \textbf{MUST NOT} perform catalog mutation after Phase C
completes. A lazily-expanded include that would introduce a package not
already in the frozen package set is therefore \texttt{PKG-017} (late
package binding after catalog freeze) --- the same code used for any other
attempt to contribute a package after the freeze. This closes the gap that
would otherwise exist between ``lazy for execution'' and ``lazy for
planning'': the two are deliberately decoupled, and only the former is
permitted.
...
\hline
\texttt{FAILED}   & Unhandled error or \texttt{on\_timeout: fail} & Terminal (unless compensated in a future release). \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item Load and cache named business calendars; built-in calendars are compiled into
        the gert binary. Custom calendar definitions are deferred to a future release.
\end{itemize}
...
\paragraph{Built-in calendars for .}
...
\textbf{deferred to a future release} as a future-work item.
...
\label{sec:collector_field_validation_contract}
% ─────────────────────────────────────────────────────────────────────────────
...
load-time warning (not a hard error) if an ephemeral variable is referenced in a
...
The cancellation contract does not invoke compensation handlers. Saga/compensation
(stepping backward through a compensation chain on failure) is deferred to a future release as specified
...
The runtime is \textbf{single-run-per-process}. Each \texttt{gert exec} invocation is a
...
\begin{itemize}
  \item \textbf{One goroutine per run:} the step execution loop runs on a single goroutine.
        Steps execute sequentially; there is no goroutine-per-step parallelism.
...
\texttt{gert serve} is the HTTP server that powers the VS Code extension, the embedded
web preview, the MCP tools layer, and any future external client. It exposes a JSON-RPC 2.0
endpoint at \texttt{POST /rpc} alongside REST and Server-Sent Events (SSE) endpoints. In the
The architecture it is classified as an \textbf{adapter in the API/Adapter Layer}.

\subsection{Architectural Position}

\texttt{gert serve} is a process that:
\begin{enumerate}
  \item Starts a long-lived HTTP listener (default \texttt{:7778}) exposing JSON-RPC 2.0 at
        \texttt{POST /rpc} alongside REST endpoints (\texttt{POST /runs}, \texttt{GET /runs/\{id\}/state},
        \texttt{DELETE /runs/\{id\}}, \ldots) and Server-Sent Events streams
        (\texttt{GET /events}, \texttt{GET /runs/\{id\}/state}, \texttt{GET /runs/\{id\}/interactions}).
  \item On receiving \texttt{exec/start} (or \texttt{POST /runs}), calls \texttt{Runtime.Start()} and stores the
  ...
  \item Drains the \texttt{Events()} channel in a background goroutine and forwards each
        event to subscribed SSE clients as a named SSE event whose data payload is the
        event's JSON representation. JSON-RPC clients receive the same events as
        notifications on a long-lived RPC channel.
...
Every event emitted by the Runtime Core is forwarded to subscribed clients. Two transports
are offered concurrently against the same in-process event bus:

\begin{itemize}
  \item \textbf{SSE} (\texttt{text/event-stream}): the VS Code extension and the web preview
        attach an \texttt{EventSource} to \texttt{GET /events} (or scope-narrowed streams
        like \texttt{GET /runs/\{id\}/state} and \texttt{GET /runs/\{id\}/interactions}).
        Each event is emitted with an \texttt{id:} header carrying the event sequence so
        clients can resume with \texttt{Last-Event-ID} after a transient disconnect. A 15\,s
        comment-frame heartbeat (\texttt{:\ hb\textbackslash n\textbackslash n}) keeps idle
        connections alive through reverse proxies.
  \item \textbf{JSON-RPC notifications}: clients holding a long-lived RPC channel receive
        each event as a notification (no \texttt{id} field) with method
        \texttt{event/\textit{eventType}}. The payload is identical to the SSE data field.
\end{itemize}

A WebSocket endpoint (\texttt{GET /ws}) is exposed for byte-stream consumers that prefer
framed messages over SSE; it carries the same event schema.
...
\texttt{RunHandle} over the same session. Mid-run SSE disconnects are recovered by the
browser/EventSource layer automatically: the client reconnects with the most recent
\texttt{Last-Event-ID}, and \texttt{gert serve} replays buffered events past that
sequence number. The trace and checkpoint on disk ensure no steps are lost.
...
\begin{itemize}
  ...
  \item \textbf{Message broker transport} — delegate to a pluggable \texttt{MessageBroker}
        interface (implementation-defined for ; a no-op stub ships by default). The
