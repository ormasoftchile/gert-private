...
Gert v2 applies the three-pillar observability model to workflow execution. Each pillar serves a distinct purpose:
...
Gert v2 integrates OpenTelemetry as the standard tracing layer. All spans, metrics, and logs follow the OpenTelemetry specification~\cite{otel-spec} to ensure portability across observability backends.
...
Gert v2 propagates trace context across system boundaries using W3C Trace Context~\cite{w3c-trace-context}.
...
Gert v2 uses \textbf{parent-based sampling} with a configurable default rate:
...
Gert v2 exposes the following metrics in Prometheus OpenMetrics format. All metrics are prefixed with \texttt{gert\_}.
...
Gert v2 emits structured JSON logs to \texttt{stderr}. Every log line is a complete JSON object.
...
Gert v2 provides CLI commands for self-diagnosis and trace inspection.
...
20260418T140015-f3a7b2       rollback-v2      bob@example.com      failure    45s
...
Gert v2 provides machine-readable output for CI/CD integration.
...
This chapter specifies a production-grade observability model for gert v2 grounded in industry standards:
