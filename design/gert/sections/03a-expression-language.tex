...
GXL is the only supported expression surface for these fields.
\end{quote}
...
\texttt{str}    & Namespace prefix (string helpers); dual-role GDP root (see note) \\
\texttt{list}   & Namespace prefix (list helpers); dual-role GDP root (see note) \\
\texttt{regex}  & Namespace prefix (regex helpers); dual-role GDP root (see note) \\
\texttt{math}   & Reserved namespace prefix (v2; no methods in v1) \\
\hline
\end{tabular}
\end{center}

\noindent\textbf{Dual-role namespace keywords.}
The keywords \texttt{str}, \texttt{list}, and \texttt{regex} serve a dual role.
When immediately followed by \texttt{.}, they are committed to the
\texttt{NamespaceCall} production.
When \emph{not} followed by \texttt{.} (e.g., used as a bare variable name or
array-indexed path), they are treated as valid GDP root identifiers.
All other keywords (\texttt{and}, \texttt{or}, \texttt{not}, \texttt{len},
\texttt{now}, \texttt{math}, \texttt{true}, \texttt{false}, \texttt{null})
remain unconditionally reserved and MUST be rejected with
\texttt{GXL-PARSE-010} when used in identifier position.
See \S\ref{subsec:gxl:gdp} and conformance vectors
\texttt{TV-GXL-PARSE-028}, \texttt{TV-GXL-PARSE-048}, \texttt{TV-GXL-PARSE-083}.
...
error \texttt{GXL-PARSE-010}, with one exception: the namespace prefix
keywords \texttt{str}, \texttt{list}, and \texttt{regex} are permitted as GDP
root identifiers when not immediately followed by \texttt{.} (see
\S\ref{subsec:gxl:keywords} dual-role note and \S\ref{subsec:gxl:gdp}).
...
\paragraph{Dual-role namespace keywords as GDP roots.}
The namespace prefix keywords \texttt{str}, \texttt{list}, and \texttt{regex}
MAY serve as a GDP root identifier when the immediately following token is
\emph{not} \texttt{.} (DOT).
Under PEG ordered alternation, \texttt{Primary} tries \texttt{NamespaceCall}
before \texttt{GDP}.
When a namespace keyword is \emph{not} followed by \texttt{.},
\texttt{NamespaceCall} fails immediately (it requires a DOT after the
namespace prefix) and the parser falls through to \texttt{GDP}, treating the
keyword token as a valid root identifier.
Example: \texttt{list[10]} — \texttt{list} is not followed by \texttt{.}
→ \texttt{parse\_ok} (\texttt{TV-GXL-PARSE-028}).

When a namespace keyword IS followed by \texttt{.}, it is committed to the
\texttt{NamespaceCall} production; the GDP dual-role carve-out does
\emph{not} apply.
Example: \texttt{str.foo} — \texttt{str} followed by \texttt{.}, but no
\texttt{(} → \texttt{NamespaceCall} fails; GDP fallback inapplicable →
\texttt{GXL-PARSE-001} (\texttt{TV-GXL-PARSE-083}).

This dual-role carve-out does \emph{not} extend to \texttt{math},
\texttt{len}, \texttt{now}, or any other keyword.
...
\texttt{NamespaceCall} (which requires \texttt{(ArgList)}).
When a namespace keyword is followed by \texttt{.}, it is committed to the
\texttt{NamespaceCall} production; the GDP dual-role carve-out (see
\S\ref{subsec:gxl:gdp}) does \emph{not} apply.
Under PEG ordered alternation all \texttt{Primary} alternatives therefore fail;
...
\noindent\textbf{Equality operators are restricted to scalars and null.}
The equality operators \texttt{==} and \texttt{!=} are defined only for
scalar operands (\texttt{number}, \texttt{string}, \texttt{bool}) and
\texttt{null}.
Applying \texttt{==} or \texttt{!=} to a \texttt{list} or \texttt{object}
operand MUST raise \texttt{GXL-TYPE-001}.
Deep structural comparison of collections is not supported in GXL v1;
authors requiring it must decompose comparisons using \texttt{len} and
scalar field access.
(Source: \texttt{gxl.ebnf} \S5.2; ratification TESS-AMBIG-4, 2026-06-05.)

\noindent\textbf{Boolean ordered comparison is forbidden.}
The ordered comparison operators (\texttt{<}, \texttt{<=}, \texttt{>},
\texttt{>=}) are defined for \texttt{number} and \texttt{string} operands
only.
Applying any ordered operator to \texttt{bool} operands MUST raise
\texttt{GXL-TYPE-005} at evaluation time.
Booleans define \emph{equality} (\texttt{==}, \texttt{!=}) but no total
ordering; mapping \texttt{false}/\texttt{true} to 0/1 would constitute
implicit coercion, which GXL forbids.
(Source: \texttt{gxl.ebnf} \S5.2; ratification TESS-AMBIG-3, 2026-06-05.)
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Truthy Coercion in Boolean Context}
\label{sec:gxl:semantics:truthy}

GXL defines \emph{boolean context} as any syntactic position where the
language requires a truth value: the operand of \texttt{not}, both sides
of \texttt{and}/\texttt{or}, the condition in a runbook \texttt{when}/
\texttt{condition}/\texttt{until}/\texttt{iterate.until} field, and the
test position of a ternary expression (\texttt{x ? a : b}).

\noindent\textbf{Rule (normative):}
A value appearing in boolean context MUST have PJVM type \texttt{bool}.
If the runtime evaluator encounters a non-\texttt{bool} value in boolean
context, it MUST raise \texttt{GXL-TYPE-002} (``type error: expected bool'').
There is \textbf{no} implicit truthy/falsy coercion for any type.

\noindent This means GXL has no ``\texttt{Truthy()}'' function in the
JS or Python sense.  The evaluator's boolean-context check is a strict
type gate: \texttt{bool} passes, everything else is an error.

\medskip
\noindent\textbf{Worked examples by PJVM type:}

\begin{center}
\begin{tabular}{lll}
\hline
\textbf{Value}            & \textbf{In boolean context} & \textbf{Result} \\
\hline
\texttt{null}             & \texttt{if null:}           & \texttt{GXL-TYPE-002} \\
\texttt{false}            & \texttt{if false:}          & evaluates; condition is false \\
\texttt{true}             & \texttt{if true:}           & evaluates; condition is true \\
\texttt{0}                & \texttt{if 0:}              & \texttt{GXL-TYPE-002} \\
\texttt{1}                & \texttt{if 1:}              & \texttt{GXL-TYPE-002} \\
\texttt{-1}               & \texttt{if (-1):}           & \texttt{GXL-TYPE-002} \\
\texttt{0.0}              & \texttt{if 0.0:}            & \texttt{GXL-TYPE-002} \\
\texttt{""}               & \texttt{if "":}             & \texttt{GXL-TYPE-002} \\
\texttt{"hello"}          & \texttt{if "hello":}        & \texttt{GXL-TYPE-002} \\
\texttt{"false"}          & \texttt{if "false":}        & \texttt{GXL-TYPE-002} \\
\texttt{[]}               & \texttt{if []:}             & \texttt{GXL-TYPE-002} \\
\texttt{[0]}              & \texttt{if [0]:}            & \texttt{GXL-TYPE-002} \\
\texttt{\{\}}             & \texttt{if \{\}:}           & \texttt{GXL-TYPE-002} \\
\texttt{\{"a": 0\}}       & \texttt{if \{"a": 0\}:}    & \texttt{GXL-TYPE-002} \\
\hline
\end{tabular}
\end{center}

\noindent\textbf{Recommended idioms} for authors who need emptiness checks:

\begin{itemize}
  \item Collection emptiness: \texttt{len(myList) > 0} or \texttt{len(myList) == 0}
  \item Null check: \texttt{x != null}
  \item Numeric zero check: \texttt{n != 0}
  \item Empty string check: \texttt{s != ""}
\end{itemize}

\noindent\textbf{Cross-runtime note:}
All GXL runtime implementations (Go, C\#, TypeScript, \ldots) MUST
enforce this rule identically.  A conforming \texttt{EvalBool} function
accepts only \texttt{bool} results; any other PJVM type triggers
\texttt{GXL-TYPE-002}.  No runtime may introduce its own truthy/falsy
table.
(Ratification: Truthy arbitration, 2026-06-07.)
...
\begin{enumerate}
  ...
  \item Each subsequent dotted segment (\texttt{.field}) is applied to the
    current value.
    \begin{itemize}
      \item If the current value is \emph{not an object} (i.e., it is a
        \texttt{number}, \texttt{string}, \texttt{bool}, or \texttt{null}),
        the evaluator MUST raise \texttt{GXL-PATH-004}.
      \item If the current value \emph{is} an object but the named field is
        not present in it, the evaluator MUST raise \texttt{GXL-PATH-001}.
    \end{itemize}
    (Ratification TESS-AMBIG-5 and TESS-AMBIG-6, 2026-06-05.)
...
\hline
\texttt{GXL-TYPE-001} & Type checker & Type mismatch in comparison: comparing values of incompatible types (e.g., string~\texttt{==}~number), or applying \texttt{==}/\texttt{!=} to a \texttt{list} or \texttt{object} operand. \\
...
\texttt{GXL-TYPE-005} & Evaluator & Ordered comparison on boolean operands: \texttt{<}, \texttt{<=}, \texttt{>}, or \texttt{>=} applied to \texttt{bool} values. Booleans define equality only. \\
\hline
\multicolumn{3}{l}{\textit{Path errors (raised at evaluation time)}} \\
\hline
\texttt{GXL-PATH-001} & Evaluator & Variable or path segment not found: first \texttt{IDENT} not in context map, or named field not present in an object. \\
\texttt{GXL-PATH-002} & Evaluator & Array index out of bounds: index $\geq$ array length. \\
\texttt{GXL-PATH-003} & Evaluator & Index applied to a non-array value: e.g., \texttt{"hello"[0]} or bracket traversal through a \texttt{null} or non-array intermediate. \\
\texttt{GXL-PATH-004} & Evaluator & Field access on non-object value: dotted segment applied when the current node is not an \texttt{object} (e.g., \texttt{foo.bar} where \texttt{foo} is a number, string, boolean, or \texttt{null}). \\
\hline
