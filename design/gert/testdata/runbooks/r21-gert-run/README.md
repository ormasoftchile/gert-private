# r21-gert-run — `gert run` CLI Adapter Test Fixture

**Phase:** Phase 10 — Adapters  
**Purpose:** Integration test for standalone `gert run` CLI execution  
**Created:** 2026-04-20  
...
This fixture exercises the `gert run` CLI adapter in standalone mode (no `gert serve`, no EventDispatcher). It validates:
...
```bash
gert run testdata/runbooks/r21-gert-run/schema.yaml
```
...
```bash
gert run testdata/runbooks/r21-gert-run/schema.yaml --var environment=prod
```
...
```bash
gert run testdata/runbooks/r21-gert-run/schema.yaml --var environment=staging
```
...
2. Run: `gert run testdata/runbooks/r21-gert-run/schema.yaml`
...
The `gert run` adapter passes `nil` as the `EventDispatcher` handle to the Runtime Core. When a `wait_for_event` step is dispatched, the executor checks:

```go
if step.Type == "wait_for_event" && dispatcher == nil {
    return fmt.Errorf("step type 'wait_for_event' requires gert serve mode")
}
```
...
```go
func TestGertRunStandalone(t *testing.T) {
    tests := []struct {
        name      string
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            // Build CLI args
            args := []string{"run", "testdata/runbooks/r21-gert-run/schema.yaml"}
            for k, v := range tt.vars {
                args = append(args, "--var", fmt.Sprintf("%s=%s", k, v))
            }

            // Execute
            cmd := exec.Command("gert", args...)
...
func TestGertRunRejectsWaitForEvent(t *testing.T) {
    // Load fixture
    rb := loadYAML(t, "testdata/runbooks/r21-gert-run/schema.yaml")
    ...
    // Execute
    cmd := exec.Command("gert", "run", tmpPath)
    output, err := cmd.CombinedOutput()
    ...
    assert.Contains(t, string(output), "step type 'wait_for_event' requires gert serve mode")
}
```
...
```jsonl
{"event_id":"...","run_id":"...","runbook_id":"r21-gert-run","timestamp":"...","kind":"run/started","sequence":0,"payload":{...}}
{"event_id":"...","run_id":"...","runbook_id":"r21-gert-run","timestamp":"...","kind":"step/started","sequence":1,"payload":{"step_id":"capture_env"}}
{"event_id":"...","run_id":"...","runbook_id":"r21-gert-run","timestamp":"...","kind":"step/completed","sequence":2,"payload":{"step_id":"capture_env","status":"success","captured":{"environment":"dev"}}}
...
{"event_id":"...","run_id":"...","runbook_id":"r21-gert-run","timestamp":"...","kind":"run/completed","sequence":N,"payload":{"status":"success"}}
```
...
```
testdata/runbooks/r21-gert-run/
...
- [ ] `gert run` CLI parses `--var` flags correctly
...
- [ ] `wait_for_event` rejected with correct error message in `gert run` mode
...
- Phase 10 Audit: `/Volumes/Projects/gert/.squad/tmp/barbara-phase10-audit.md`
- Architectural Decisions: `/Volumes/Projects/gert/.squad/decisions/inbox/barbara-phase10-audit.md`
