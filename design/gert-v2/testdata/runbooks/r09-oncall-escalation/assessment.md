...
|--------|-------|----------|-------------|
| G3-001 | G3 | HIGH | Cross-iterate convergence not supported — once any iterate (steps 3-6) succeeds, remaining should be skipped; workaround uses `when: '{{ ne .acknowledged "true" }}'` guard on each iterate block |
...
1. **Steps 3-6 (Escalation Loops):** The prose says "step through primary, secondary, manager,
   ...
   is guarded with `when: '{{ ne .acknowledged "true" }}'` to skip if already acknowledged.
...
3. **Step 7 (Emergency Broadcast):** Guarded with `when: '{{ ne .acknowledged "true" }}'`.
