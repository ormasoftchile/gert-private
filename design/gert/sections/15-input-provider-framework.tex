...
\begin{tikzpicture}[scale=0.85, transform shape]
  % ── Main resolution pipeline (left to right) ─────────────────────────
  \node[stepbox, minimum width=2.4cm] (binding)
    at (0,0) {\texttt{from:}\\binding};
  \node[stepbox, minimum width=2.4cm, right=1.6cm of binding] (prefix)
    {Prefix\\Match};
  \node[stepbox, minimum width=2.4cm, right=1.6cm of prefix] (start)
    {Start\\Provider};
  \node[stepbox, minimum width=2.4cm, right=1.6cm of start] (batch)
    {Batch\\Collect};
  \node[stepbox, minimum width=2.4cm, right=1.6cm of batch] (rpc)
    {JSON-RPC\\Resolve};
  \node[stepbox, minimum width=2.4cm, right=1.6cm of rpc] (merge)
    {Merge\\Vars};

  % ── Main flow ────────────────────────────────────────────────────────
  \draw[gertarrow] (binding) -- (prefix);
  \draw[gertarrow] (prefix)  -- (start);
  \draw[gertarrow] (start)   -- (batch);
  \draw[gertarrow] (batch)   -- (rpc);
  \draw[gertarrow] (rpc)     -- (merge);

  % ── Fallback: no prefix match ─────────────────────────────────────────
  \node[stepbox, fill=black!10, minimum width=2.4cm,
    below=1.4cm of prefix] (prompt) {Interactive\\Prompt};
  \draw[gertarrow, dashed] (prefix) -- (prompt)
    node[midway, right, font=\scriptsize] {no match};

  % ── Fallback: unresolved binding ──────────────────────────────────────
  \node[stepbox, fill=black!10, minimum width=2.4cm,
    below=1.4cm of rpc] (fallback) {fallback:\\field value};
  \draw[gertarrow, dashed] (rpc) -- (fallback)
    node[midway, right, font=\scriptsize] {unresolved};

  % ── Success terminal ──────────────────────────────────────────────────
  \node[catbox, minimum width=2.4cm, right=1.6cm of merge] (done)
    {Value\\Available};
  \draw[gertarrow] (merge) -- (done);

  % ── Fallbacks also route to done ─────────────────────────────────────
  \draw[gertarrow, dashed] (prompt)   -| (done);
  \draw[gertarrow, dashed] (fallback) -| (done);

  % ── "already running" bypass ─────────────────────────────────────────
  \draw[gertarrow, black!40]
    (prefix) to[out=90, in=90]
    node[above, font=\scriptsize] {already running} (batch);
\end{tikzpicture}
\caption{Input provider resolution pipeline. A \texttt{from:} binding triggers
  a prefix lookup; if a matching provider is found its process is started (or
  reused), inputs are batched, and a single \texttt{provider/resolve} JSON-RPC
  call returns all values.  Unresolved bindings fall back to the
  \texttt{fallback:} field or an interactive prompt.}
\label{fig:provider-resolution-flow}
\end{figure}
...
\begin{itemize}
  ...
  \item \texttt{multiple: true} (optional, default \texttt{false}) — when set, the
        provider MUST allow the operator to select one or more values.  The
        \texttt{selected} response field MUST be a JSON array of strings rather than a
        single string.  The run engine stores the result as a JSON array of strings; see
        §\ref{sec:collector_field_validation} for storage semantics.
  \item \texttt{options\_from} (optional) — mutually exclusive with the static
        \texttt{options} array.  When present, the run engine calls
        \texttt{inputProvider/getOptions} to fetch options dynamically before dispatching
        the \texttt{provider/choice} request; see §\ref{sec:dynamic-options-protocol}.
\end{itemize}
...
A \texttt{decision} step presents the
...
\begin{itemize}
  \item The \texttt{fields} array declares the form structure.  Each field has:
        \begin{itemize}
          ...
          \item \texttt{type}: one of \texttt{text}, \texttt{number},
                \texttt{integer}, \texttt{date}, \texttt{datetime},
                \texttt{boolean}, \texttt{select}, \texttt{file},
                \texttt{image}, or \texttt{url}.  Full per-type validation and
                variable-storage rules are defined in
                §\ref{sec:collector_field_validation}.
          \item \texttt{label}: human-readable prompt text.
          \item \texttt{required}: boolean flag.
          \item \texttt{visible}: boolean flag, pre-evaluated by the executor from
                the field's \texttt{when} expression before the request is sent.
                \texttt{true} means the field should be rendered; \texttt{false}
                means the field should be hidden or skipped.  CLI providers SHOULD
                skip fields where \texttt{visible: false}; rich UI providers SHOULD
                show or hide the field dynamically as the user fills in earlier
                fields, re-evaluating \texttt{when} client-side so the form responds
                in real time without a round-trip to the executor.  If \texttt{when}
                was absent on the field, \texttt{visible} is always \texttt{true}.
                See §\ref{sec:conditional-field-eval} for the full conditional field
                evaluation contract.
          \item \texttt{validation} (optional object): client-side validation
                constraints that providers MAY enforce before submitting values,
                reducing unnecessary round-trips.  The executor always re-validates
                server-side regardless of whether the provider enforced them:
                \begin{itemize}
                  \item \texttt{validation.pattern} — RE2 regex that the value must
                        fully match (\texttt{text} fields only).
                  \item \texttt{validation.patternHint} — human-readable message
                        shown when \texttt{pattern} fails (e.g.\ \emph{``Must be
                        a valid ticket reference: ABC-1234''}).
                  \item \texttt{validation.minLength},
                        \texttt{validation.maxLength} — character-count bounds for
                        \texttt{text} fields.
                  \item \texttt{validation.min}, \texttt{validation.max},
                        \texttt{validation.step} — numeric bounds for
                        \texttt{number} and \texttt{integer} fields.
                  \item \texttt{validation.min}, \texttt{validation.max} — date
                        bounds for \texttt{date} and \texttt{datetime} fields
                        (ISO~8601 strings; lexicographic comparison is correct
                        for ISO dates).
                  \item \texttt{options} array or \texttt{options\_from} binding
                        — required for \texttt{select} fields; see
                        §\ref{sec:dynamic-options-protocol}.
                  \item \texttt{multiple: true|false} (default \texttt{false})
                        — enables multi-select for \texttt{select} fields.
                  \item \texttt{validation.min\_selections},
                        \texttt{validation.max\_selections} — array length
                        bounds for multi-select fields.
                  \item \texttt{multiline: true|false} — UI hint for
                        \texttt{text} fields; no effect on validation or
                        variable storage.
                  \item \texttt{accept} (MIME types) and \texttt{maxSizeBytes}
                        — for \texttt{file} and \texttt{image} fields.
                \end{itemize}
        \end{itemize}
...
a file path and copies the file to the local artifact store.  For
\texttt{type: select}, it presents a numbered list (single-select) or a
checkbox list (multi-select).  For \texttt{type: boolean}, it prompts with a
\texttt{[y/N]} confirmation.  For \texttt{type: number}, \texttt{integer},
\texttt{date}, and \texttt{datetime}, it accepts free-text input and validates
the format before accepting.  In \texttt{dry-run} mode, all values are read
from the scenario file.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Dynamic Options Protocol}
\label{sec:dynamic-options-protocol}
% ─────────────────────────────────────────────────────────────────────────────

When a \texttt{select} field in a \texttt{collector} step, or a \texttt{choice} step,
declares \texttt{options\_from.provider}, the run engine must fetch the option list
dynamically from that provider before rendering the field or dispatching the interactive
step request.

\subsubsection*{Fetch Sequence}

\begin{enumerate}
  \item \textbf{Provider lookup} — the run engine resolves the provider ID from
        \texttt{options\_from.provider} in \texttt{ExecutionPlan.Providers}.  If the
        provider is not registered, the step fails immediately with
        \texttt{options\_from.unknown\_provider}.
  \item \textbf{Provider start} — if the provider process is not yet running, it is
        started using the normal \texttt{stdio-jsonrpc} startup sequence.
  \item \textbf{Options request} — the run engine sends an
        \texttt{inputProvider/getOptions} JSON-RPC request to the provider, including
        the current variable scope so the provider can filter or compute options
        contextually.
  \item \textbf{Option list received} — the run engine treats the returned
        \texttt{options} array as if it had been declared statically in the runbook.
        Validation and rendering proceed identically to the static-options path.
\end{enumerate}

Failure to fetch options (provider error, network error, or timeout) fails the step
immediately with \texttt{options\_from.fetch\_failed}.  There is no silent fallback to
an empty options list.

\subsubsection*{\texttt{inputProvider/getOptions} Request}
...
\paragraph{Request fields.}
\begin{itemize}
  \item \texttt{providerId} — the provider ID declared in
        \texttt{options\_from.provider}.
  \item \texttt{field} — the \texttt{options\_from.field} value; allows a single
        provider to serve multiple distinct option lists (e.g.\ \texttt{managers},
        \texttt{engineers}, \texttt{directors}).
  \item \texttt{stepId} — the step making the request (for audit and debugging).
  \item \texttt{variables} — the current run variable scope at the time the step is
        reached.  The provider MAY use these to filter or compute options contextually
        (e.g.\ return only managers in the operator's department).
  \item \texttt{context} — standard execution context (run ID, actor, mode).
\end{itemize}

\subsubsection*{\texttt{inputProvider/getOptions} Response}
...
\paragraph{Response fields.}
\begin{itemize}
  \item \texttt{options} — array of \texttt{\{value, label\}} objects.  Each
        \texttt{value} is the string stored in the run variable on selection;
        \texttt{label} is the human-readable text.  An optional \texttt{hint} field
        provides secondary descriptive text.
  \item \texttt{cacheTtlSeconds} (optional) — if non-zero, the run engine caches the
        returned option list in memory for the remainder of the run (capped at the
        declared TTL).  This avoids repeated RPC calls when the same dynamic field
        appears in multiple steps of a single run.  A value of \texttt{0} disables
        caching for this response.
\end{itemize}

\paragraph{Capability declaration.}
Providers that implement \texttt{inputProvider/getOptions} MUST declare the capability
in their handshake response.  If the run engine encounters an \texttt{options\_from}
binding and the matched provider does not advertise \texttt{getOptions: true}, the step
fails with \texttt{options\_from.capability\_not\_supported}.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Autocomplete Search Protocol}
\label{sec:autocomplete-search-protocol}
% ─────────────────────────────────────────────────────────────────────────────

The \texttt{type: autocomplete} field in a \texttt{collector} step requires live-search
support from the input provider.  The run engine issues \texttt{inputProvider/search}
RPC calls as the operator types, with debouncing handled by the UI layer.

\subsubsection*{\texttt{inputProvider/search} Request}
...
\paragraph{Request fields.}
\begin{itemize}
  \item \texttt{provider} — the provider ID from \texttt{options\_from.provider} (or
        the field's dedicated \texttt{autocomplete.provider} binding).
  \item \texttt{field} — the field ID within the step, allowing a single provider to
        serve multiple autocomplete fields.
  \item \texttt{query} — the current text typed by the operator.  May be an empty
        string \texttt{""} on initial focus or during CLI degradation (see below).
  \item \texttt{minChars} — the minimum character count declared on the field.  The
        run engine does not issue the RPC until \texttt{len(query) >= minChars}; this
        field is informational for the provider.
  \item \texttt{runId} — the current run ID (for audit and logging on the provider side).
  \item \texttt{variables} — the current run variable scope at dispatch time.  Providers
        MAY use this to filter results contextually (e.g., return only services in the
        operator's selected region).
\end{itemize}

\subsubsection*{\texttt{inputProvider/search} Response}
...
\paragraph{Response fields.}
\begin{itemize}
  \item \texttt{results} — array of \texttt{\{value, label\}} objects.  \texttt{value}
        is the string stored in the run variable on selection; \texttt{label} is the
        human-readable text shown in the UI.
  \item \texttt{hasMore} (boolean) — if \texttt{true}, the UI SHOULD display a hint
        such as \emph{``Showing top N results — keep typing to narrow.''}.  A
        \texttt{false} value indicates the full matching set is returned.
\end{itemize}

\paragraph{Timeout contract.}
Providers MUST return within \textbf{500\,ms}.  If no response is received within this
window, the run engine treats the call as timed out, returns an empty results list, and
allows the operator to retry by continuing to type.  The timeout is measured from the
moment the RPC request is sent on the transport.

\paragraph{CLI degradation.}
The built-in \texttt{prompt} provider does not support real-time search.  When an
\texttt{autocomplete} field is dispatched to a CLI provider, the run engine issues a
single \texttt{inputProvider/search} call with \texttt{query: ""} to retrieve all
available options, then presents the result as a standard \texttt{select} list.  Typing
ahead is not supported in CLI mode.

\paragraph{Capability declaration.}
Providers that implement \texttt{inputProvider/search} MUST declare \texttt{search: true}
in their capability advertisement (see matrix below).  A provider that does not advertise
\texttt{search: true} will have autocomplete fields degraded to the CLI path automatically.
...
\hline
\textbf{Provider} & \textbf{Input Resolution} & \textbf{choice} & \textbf{decision} & \textbf{collector} & \textbf{getOptions} & \textbf{search} \\
\hline
\texttt{prompt}   & Yes & Yes & Yes & Yes & No  & No  \\
\texttt{env}      & Yes & No  & No  & No  & No  & No  \\
\texttt{file}     & Yes & No  & No  & No  & No  & No  \\
\texttt{workspace}& Yes & No  & No  & No  & No  & No  \\
\hline
\end{tabular}
\end{center}

\noindent
\texttt{search: false} for all built-in providers reflects CLI degradation: autocomplete
fields degrade to a standard select when the active provider cannot perform live search.
Rich UI providers (e.g., the VS~Code extension's built-in provider) SHOULD declare
\texttt{search: true} to enable real-time autocomplete rendering.
