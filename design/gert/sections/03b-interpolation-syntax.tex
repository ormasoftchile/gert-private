...
GIS replaces the prior Go \texttt{text/template} interpolation surface;
see \S\ref{sec:gis:migration} for the migration note.
...
Fixture migration is tracked under Stream~D.
\end{quote}
...
GIS replaces the prior Go \texttt{text/template} interpolation surface that
was used for string-typed fields
(\texttt{display.content}, \texttt{args}, \texttt{stdin},
and related fields) in earlier versions of the GERT schema.
The key authoring differences are:

\begin{itemize}
  \item The interpolation delimiter changes from \texttt{\{\{.Variable\}\}}
    (Go template syntax) to \texttt{\$\{variable\}} (GIS syntax).

  \item The canonical escape for a literal dollar-brace sequence is
    \texttt{\textbackslash\$\{} (single backslash; OI-GIS-01, ratified).
    Any historical occurrences of the deprecated \texttt{\$\$\{} doubling
    convention (proposal \S2.6, now superseded) MUST be converted to
    \texttt{\textbackslash\$\{} by the Stream~E migration tool.

  \item GIS allows full GXL expressions inside \texttt{\$\{...\}},
    not only bare variable paths.  Authors MAY use \texttt{str.*} and
    \texttt{list.*} functions, arithmetic, and comparisons directly
    within interpolation blocks.

  \item Missing variables are hard errors under GIS\@.
    The Go \texttt{text/template} behavior of silently substituting an
    empty string for a missing key is \textbf{removed}.
\end{itemize}

A full deprecation appendix with migration rewrite rules will be added in
a later Stream~B task.
Existing spec sections that still reference Go \texttt{text/template}
syntax are identified for rewrite in the Stream~B backlog
(\texttt{02-architecture.tex} lines 467--477 and 586--587;
\texttt{03-schema-vnext.tex} lines 2518--2560).
