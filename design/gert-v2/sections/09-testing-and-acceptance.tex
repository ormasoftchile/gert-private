...
This section specifies the complete testing strategy for gert v2: the test framework, unit and contract test patterns, integration test strategy, the \texttt{gert test} feature, regression testing against the v1 runbook corpus, performance acceptance criteria, the extension test harness, and CI/CD gate requirements.
...
All gert v2 tests are written in Go using the standard \texttt{testing} package as the test runner and \texttt{github.com/stretchr/testify}~\cite{testify-library} for assertions:
...
Each Go package in the v2 module ships a co-located \texttt{\_test.go} file (white-box tests) and optionally an \texttt{<pkg>\_integration\_test.go} file (integration tests, gated by a build tag). The naming convention:
...
\subsection{v1 Scenario Compatibility}

v1 scenario files (\texttt{commands}/\texttt{evidence} structure) are forward-compatible with v2 without modification. The v2 parser accepts both the v1 format and the extended v2 format (which adds the \texttt{assertions} block). Runbook authors can migrate incrementally by adding \texttt{test.yaml} alongside existing v1 scenarios.

% ------------------------------------------------------------------ %
\section{Contract Tests}
% ------------------------------------------------------------------ %

Contract tests verify the interface boundaries between v2's five components. Each contract test lives in the \emph{consumer} package and uses a conformance helper that validates any implementation satisfying the interface.
...
\begin{itemize}
  \item Every \texttt{apiVersion: runbook/v2} document in \texttt{testdata/} must pass structural validation without errors.
...
\subsection{v1 Runbook Corpus as Fixtures}

The v1 runbook corpus (located in \texttt{examples/} and operator-contributed runbooks) serves as a regression suite for v2 compatibility. The regression test runner:

\begin{enumerate}
  \item Reads each v1 runbook from the corpus.
  \item Runs \texttt{gert validate} (v2 schema) and asserts it passes (or passes after migration).
  \item For runbooks with associated v1 scenario files, runs \texttt{gert test} and asserts the outcome matches the v1 golden output.
  \item Fails the build if any previously passing corpus runbook regresses.
\end{enumerate}

Migration compatibility is measured as: the percentage of v1 corpus runbooks that pass v2 \texttt{gert validate} without modification. The v2 launch target is $\geq$95\%.
...
The following acceptance criteria must be satisfied before a v2 release is considered shippable:
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item The v1 runbook corpus passes \texttt{gert validate} at $\geq$95\% without modification.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Schema validation}: validate all YAML fixtures in \texttt{testdata/} against v2 JSON Schema. Target: $<$10 s.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Regression corpus}: \texttt{gert test} against v1 corpus. Target: $<$2 min.
