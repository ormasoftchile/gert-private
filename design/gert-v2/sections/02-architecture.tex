...
\texttt{text} fields support optional length and pattern constraints applied
\emph{after} the required check.  The full sequence for a \texttt{text} field is:
...
\begin{enumerate}
  ...
  \item \textbf{Re-prompt} — any failure at steps 2--4 triggers a re-prompt (same
        3-attempt limit as the general algorithm).
\end{enumerate}
