...
\texttt{schema.apiVersion-suffix} & No & The suffix appended to \texttt{runbook/v2+} in runbooks authored with this Kit (e.g., \texttt{"compliance/v1"}). \\
...
Lowered \texttt{runbook/v2} YAML on stdout:
...
\begin{description}
  \item[\texttt{0}] --- Success. Stdout contains valid \texttt{runbook/v2} YAML.
...
\paragraph{Plan-time validators:} Lowered \texttt{runbook/v2} YAML on stdin.
...
\textbf{Problem:} Compiler produces invalid \texttt{runbook/v2} YAML that gert
rejects.

\textbf{Solution:} Always validate lowered output against the core
\texttt{runbook/v2} JSON Schema as part of your test suite.
