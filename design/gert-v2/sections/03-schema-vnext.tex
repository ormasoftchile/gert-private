...
\texttt{collector}        & Operator provides unstructured input/files (\S\ref{subsec:step-collector}) \\
...
Prompts the user to provide unstructured input: free-form text, file attachments,
screenshots, or multi-field forms.  The collected data is stored as named
variables or artifacts for downstream steps or evidence capture.  This replaces
the overly-generic \texttt{manual} step type from early v2 drafts.
...
\texttt{fields}     & array   & Yes & List of field objects to collect \\
...
\texttt{type}     & enum    & Yes & \texttt{text}, \texttt{multiline}, \texttt{file}, \texttt{image}, \texttt{url} \\
...
\begin{itemize}
  \item \texttt{text}: Single-line text input.
  \item \texttt{multiline}: Multi-line text area (e.g., for notes or summaries).
  \item \texttt{file}: File attachment; stored as artifact with SHA256 hash.
  \item \texttt{image}: Image/screenshot attachment; stored as artifact.
  \item \texttt{url}: URL input with basic validation (must be valid URL).
\end{itemize}
