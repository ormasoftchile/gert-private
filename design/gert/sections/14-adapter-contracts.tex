...
Gert decouples the core runtime from presentation adapters (TUI, Web, VS
...
This chapter defines the four primary adapter surfaces exposed by gert:
...
Gert exposes four distinct adapter surfaces, each with its own versioning
and stability guarantees.

\subsection{JSON-RPC Execution Contract (\texttt{exec/v1})}
...
\paragraph{Contract version:} \texttt{exec/v1}
...
\hline
\end{tabular}
\caption{JSON-RPC execution contract methods (\texttt{exec/v1}).}
...
Within the \texttt{exec/v1} contract:
...
\subsection{WebSocket Event Stream Contract (\texttt{events/v1})}
...
\paragraph{Contract version:} \texttt{events/v1}
...
Within the \texttt{events/v1} contract:
...
\subsection{Tool Invocation Contract (\texttt{tool/v1})}
...
\paragraph{Contract version:} \texttt{tool/v1}
...
Within the \texttt{tool/v1} contract:
...
\subsection{Extension Handshake Contract (\texttt{extension/v1})}
...
\paragraph{Contract version:} \texttt{extension/v1}
...
Within the \texttt{extension/v1} contract:
...
\texttt{exec/v1}, \texttt{events/v1}).  There is no minor version: any breaking
...
Breaking changes require a new contract version (e.g., \texttt{exec/v1} →
\texttt{exec/v2}).
...
\begin{itemize}
  \item 2026-04-01: \texttt{exec/v1} released.
  \item 2027-06-01: \texttt{exec/v2} released; \texttt{exec/v1} enters
        deprecation.
  \item 2028-06-01: \texttt{exec/v1} removed; only \texttt{exec/v2} supported.
\end{itemize}
...
reference implementation of the tool invocation contract (\texttt{tool/v1}).
...
Gert commits to the following stability guarantees for all adapter
contracts:
...
\begin{itemize}
  \item \texttt{specs/contracts/exec-v1.openapi.yaml}
  \item \texttt{specs/contracts/events-v1.openapi.yaml}
\end{itemize}
