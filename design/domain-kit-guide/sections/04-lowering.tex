...
core \texttt{runbook/v2} YAML. The Kit compiler is a pure function:

\begin{verbatim}
Kit YAML (stdin) → Kit Compiler → Core YAML (stdout)
\end{verbatim}

\paragraph{Input:} A runbook with \texttt{apiVersion: runbook/v2+<kit-suffix>}
that uses Kit-specific step types (e.g., \texttt{approval-request},
\texttt{evidence-capture}).

\paragraph{Output:} A valid \texttt{runbook/v2} document that uses only core
...
\paragraph{Stdout:} Lowered \texttt{runbook/v2} YAML (if successful) OR error
...
func lowerRunbook(rb map[string]interface{}) (map[string]interface{}, error) {
  // Change apiVersion from runbook/v2+compliance/v1 to runbook/v2
  rb["apiVersion"] = "runbook/v2"
...
\begin{enumerate}
  ...
  \item Validate the lowered output against the core \texttt{runbook/v2} schema.
\end{enumerate}
